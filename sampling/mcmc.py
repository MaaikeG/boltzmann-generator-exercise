...
def get_step_from_uniform(range_min, range_max, size):
    return (range_max - range_min) * torch.rand(size) + range_min
...
class MCMC:
    ...
    def get_trajectory(self, potential, beta=1.0, r_initial=None):
        ...
        if r_prev is None:
            r_prev = torch.tensor(
                [get_step_from_uniform(self.sampling_range[d][0], self.sampling_range[d][1], 1) for d in
                 range(self.d)]
            )
        ...
        for n in range(self.n_steps):
            ...
            r = r_prev + get_step_from_uniform(-self.max_step, self.max_step, self.d)

            while (r < self.sampling_range[:, 0]).any() or (r > self.sampling_range[:, 1]).any():
                r = r_prev + get_step_from_uniform(-self.max_step, self.max_step, self.d)

            delta = potential(r) - potential(r_prev)
            if delta > 0:
                # print p(delta)
